...
This repo holds the [Open Policy Agent](https://www.openpolicyagent.org/) authorization rules used as replacement of the INDIGO IAM Scope Policy engine.
...
The list of policies (i.e. _data.json_ file) must be defined and built as a separate bundle (see below).
...
Before to run the compose, you need to build the policy bundle contained in the [data.json](../opa/policies/data.json) file.
...
and build the policy bundle with

```bash
$ ./opa-cli build -b opa/policies -o policy-bundle.tar.gz
```

It is gitignored so you will not need to commit anything.
...
You can even run OPA locally, without docker-compose.
